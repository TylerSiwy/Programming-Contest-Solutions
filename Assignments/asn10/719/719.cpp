// Author: Tyler Siwy
// Date: March 22, 2020
// Approach: Created the suffix array by appending the input string to itself, 
// then compute the LCP array in order to find the lexographically smallest 
// answer by checking if it's lcp is smaller than the length of the string. 
#include <algorithm>
#include <climits>
#include <iomanip>
#include <iostream>
...
int main() {
    ...
    for(int h=0; h < cases; h++){
        ...
        int sarray[20000];
        int lcp[20000];
        string dubInput = input + input;
        build_sarray(dubInput, sarray);
        compute_lcp(dubInput, sarray, lcp);
        int loc;
        for(unsigned int i = 0; i < dubInput.length(); i++){
            if(sarray[i] < input.length()){
                loc = i;
                break;
            }
        }
        for(unsigned int i = loc + 1; i < dubInput.length(); i++){
            if(lcp[i] >= dubInput.length()/2 && sarray[i] < sarray[loc])
                loc = i;
            else
                break;
        }
        cout << sarray[loc] + 1 << endl;
    }
  return 0;
}
