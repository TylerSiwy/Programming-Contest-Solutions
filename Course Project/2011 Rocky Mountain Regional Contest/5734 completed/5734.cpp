// Author: Tyler Siwy
// Date: March 3, 2020
// Approach: Process the string and save the processed version then use a suffix
// array to find the smallest cyclic rotation of the string
#include <algorithm>
#include <climits>
#include <iomanip>
#include <iostream>
...
string fDiff(string input);
int ctoi(char input) { return input - '0'; }

int main() {
  string input;
  while (cin >> input && cin) {
    int sarray[600000];
    string chain = fDiff(input);
    string tempChain = chain + chain;
    build_sarray(tempChain, sarray);
    cout << tempChain.substr(sarray[1], chain.length()) << endl;
  }
  return 0;
}
...
string fDiff(string input) {
  ...
  for (unsigned int i = 0; i < input.length(); i++)
    result += to_string(
        (ctoi(input[(i + 1) % input.length()]) - ctoi(input[i]) + 8) % 8);
  return result;
}
